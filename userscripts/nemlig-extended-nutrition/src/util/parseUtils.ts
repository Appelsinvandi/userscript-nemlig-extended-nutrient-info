...
export const parseIngredients = () => {
  const ingredientsElement = document.querySelector<HTMLDivElement>(
    'product-detail-declaration div.product-detail__declaration-label'
  )
...
export const parseNutrientDeclarations: () => NutritionDeclaration | null =
  () => {
    const nutritionDeclarationTableElement = document.querySelector(
      'product-detail-declaration table.table'
    )
    if (nutritionDeclarationTableElement == null) return null

    const rawDeclarationsArr = Array.from(
      nutritionDeclarationTableElement.querySelectorAll('tr.table__row')
    )
      .map((e) =>
        Array.from(
          e.querySelectorAll<HTMLTableDataCellElement>('td.table__col')
    ...
    function processResEntry(
      name: string,
      value?: string | number
    ): { [key: string]: number }
    function processResEntry<T extends object>(
      name: string,
      value?: T
    ): T | null
    function processResEntry(
